	...
	setup(__props, { emit: __emit }) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			if (isSystem.value) _push(`<div${ssrRenderAttrs(mergeProps({ class: "flex justify-center my-2" }, _attrs))}><span class="text-xs italic text-arbor-sage bg-arbor-glass/30 px-3 py-1 rounded-full">${ssrInterpolate(__props.message.body)}</span></div>`);
				...
				if (canBan.value) _push(`<button class="ml-2 text-arbor-sage/70 hover:text-arbor-amber text-xs"> Exclure </button>`);
				else _push(`<!---->`);
				if (canDelete.value) _push(`<button class="ml-2 text-arbor-sage/70 hover:text-red-400 text-xs"> Suppr. </button>`);
				else _push(`<!---->`);
				_push(`</div></div>`);
			}
		};
