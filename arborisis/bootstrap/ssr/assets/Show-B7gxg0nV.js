import { n as usePlayerStore, t as _sfc_main$7 } from "./GuestLayout-pnlb6vqh.js";
import { t as _sfc_main$8 } from "./Breadcrumb-DSG-dgrc.js";
import { t as _sfc_main$9 } from "./FollowButton-B84UoGmY.js";
...
var _sfc_main$5 = {
	...
	setup(__props, { emit: __emit }) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<div${ssrRenderAttrs(mergeProps({ class: "space-y-3" }, _attrs))}><div class="flex gap-3"><div class="w-8 h-8 overflow-hidden rounded-full bg-arbor-moss/30 flex items-center justify-center shrink-0">`);
			if (getAvatarUrl(__props.comment.user)) _push(`<img${ssrRenderAttr("src", getAvatarUrl(__props.comment.user))}${ssrRenderAttr("alt", `Avatar de ${__props.comment.user?.name ?? "Anonyme"}`)} class="h-full w-full object-cover" loading="lazy">`);
			else _push(`<span class="text-xs font-bold text-arbor-emerald">${ssrInterpolate(__props.comment.user?.name?.charAt(0)?.toUpperCase() ?? "?")}</span>`);
			_push(`</div><div class="flex-1 min-w-0"><div class="glass-card p-3"><div class="flex items-center justify-between mb-1"><div class="flex items-center gap-2"><span class="text-sm font-medium text-arbor-cream">${ssrInterpolate(__props.comment.user?.name ?? "Anonyme")}</span><span class="text-xs text-arbor-sage">${ssrInterpolate(formatDate(__props.comment.created_at))}</span></div>`);
			...
			if (showReplies.value && __props.comment.replies?.length > 0) {
				...
				ssrRenderList(__props.comment.replies, (reply) => {
					_push(`<div class="flex gap-3"><div class="w-6 h-6 overflow-hidden rounded-full bg-arbor-moss/20 flex items-center justify-center shrink-0">`);
					if (getAvatarUrl(reply.user)) _push(`<img${ssrRenderAttr("src", getAvatarUrl(reply.user))}${ssrRenderAttr("alt", `Avatar de ${reply.user?.name ?? "Anonyme"}`)} class="h-full w-full object-cover" loading="lazy">`);
					else _push(`<span class="text-xs font-bold text-arbor-emerald">${ssrInterpolate(reply.user?.name?.charAt(0)?.toUpperCase() ?? "?")}</span>`);
					_push(`</div><div class="flex-1"><div class="glass-card p-3"><div class="flex items-center justify-between mb-1"><div class="flex items-center gap-2"><span class="text-sm font-medium text-arbor-cream">${ssrInterpolate(reply.user?.name ?? "Anonyme")}</span><span class="text-xs text-arbor-sage">${ssrInterpolate(formatDate(reply.created_at))}</span></div>`);
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (showForm.value) {
				...
				_push(`<form class="space-y-3"><textarea rows="3" class="w-full rounded-lg bg-arbor-deep border-arbor-glass text-arbor-cream placeholder-arbor-sage/50 focus:border-arbor-emerald focus:ring-arbor-emerald text-base resize-none" placeholder="Votre commentaire...">${ssrInterpolate(unref(form).body)}</textarea>`);
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (show.value) {
				...
				_push(`</div><div><label class="block text-sm text-arbor-sage mb-2">Description (optionnel)</label><textarea rows="3" class="w-full rounded-lg bg-arbor-deep border-arbor-glass text-arbor-cream placeholder-arbor-sage/50 focus:border-arbor-emerald focus:ring-arbor-emerald text-base resize-none" placeholder="Décrivez le problème...">${ssrInterpolate(unref(form).description)}</textarea>`);
			...
			if ([
				...
				if (__props.analysis.birdnet_detections?.length > 0) {
					...
					ssrRenderList(__props.analysis.birdnet_detections, (det, i) => {
						...
						_push(`</div>`);
						if (det.confidence !== null && det.confidence !== void 0) _push(`<div class="w-20"><div class="h-1.5 rounded-full bg-arbor-glass overflow-hidden"><div class="${ssrRenderClass([confidenceColor(det.confidence), "h-full rounded-full transition-[width]"])}" style="${ssrRenderStyle(`width: ${Math.round(det.confidence * 100)}%`)}"></div></div><p class="text-[10px] text-arbor-sage text-right mt-0.5">${ssrInterpolate(Math.round(det.confidence * 100))}%</p></div>`);
						else _push(`<!---->`);
						_push(`</div>`);
					});
					_push(`<!--]--></div></div>`);
...
var _sfc_main = {
	...
	setup(__props) {
		...
		const isOwner = computed(() => page.props.auth.user && page.props.auth.user.id === props.sound.user_id);
		const isCurrentInPlayer = computed(() => player.currentSound?.id === props.sound.id);
		const archiveCode = computed(() => {
			const raw = props.sound.recorded_at || props.sound.created_at || (/* @__PURE__ */ new Date()).toISOString();
			return `ARB-${new Date(raw).getFullYear()}-${String(props.sound.id || "0000").padStart(4, "0")}`;
		});
		const creatorAvatarUrl = computed(() => props.sound.user?.avatar_url || props.sound.user?.profile?.avatar_url || props.sound.user?.profile?.avatarUrl || null);
			...
			_push(ssrRenderComponent(_sfc_main$7, null, {
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						...
						_push(`<div class="mb-8 trace-frame p-5 sm:p-8"${_scopeId}><div class="relative z-10 grid gap-8 lg:grid-cols-[1fr_0.78fr] lg:items-end"${_scopeId}><div${_scopeId}><p class="atlas-kicker mb-4"${_scopeId}>Fiche d&#39;archive vivante · ${ssrInterpolate(archiveCode.value)}</p><h1 class="atlas-heading text-5xl sm:text-6xl"${_scopeId}>${ssrInterpolate(__props.sound.title)}</h1><p class="mt-5 max-w-2xl text-arbor-sage leading-7"${_scopeId}>${ssrInterpolate(__props.sound.description || "Une trace sonore publiée dans l’atlas Arborisis, à écouter comme un fragment de territoire.")}</p></div><div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-1"${_scopeId}><div class="rounded-lg border border-arbor-mineral/10 bg-arbor-ink/40 p-4"${_scopeId}><div class="text-xs text-arbor-sage"${_scopeId}>Créateur</div>`);
						...
						_push(`</div><div class="rounded-lg border border-arbor-mineral/10 bg-arbor-ink/40 p-4"${_scopeId}><div class="text-xs text-arbor-sage"${_scopeId}>Lieu public</div><div class="mt-1 font-display text-2xl text-arbor-cream"${_scopeId}>${ssrInterpolate(__props.sound.sound_location?.location_name || "Lieu approximé")}</div></div></div></div></div><div class="grid grid-cols-1 lg:grid-cols-3 gap-8"${_scopeId}><div class="lg:col-span-2 space-y-8"${_scopeId}><div class="aspect-[16/9] rounded-xl overflow-hidden bg-arbor-deep relative group border border-arbor-mineral/10"${_scopeId}>`);
						...
						_push(`</div><div class="trace-frame p-6 relative"${_scopeId}><div class="relative z-10"${_scopeId}><div class="flex items-center gap-4 mb-4"${_scopeId}><button${ssrRenderAttr("aria-label", effectiveIsPlaying.value ? "Pause" : "Lire")} class="${ssrRenderClass([{ "is-playing": effectiveIsPlaying.value }, "audio-play-button w-14 h-14 shrink-0 group"])}"${_scopeId}>`);
						...
						_push(`</button><div class="flex-1 min-w-0"${_scopeId}><h2 class="font-display text-xl font-bold text-arbor-cream truncate"${_scopeId}>${ssrInterpolate(__props.sound.title)}</h2>`);
						_push(ssrRenderComponent(unref(Link), {
							href: _ctx.route("creators.show", __props.sound.user?.slug),
						...
						if (isOwner.value) {
							_push(`<div class="glass-card p-6"${_scopeId}><div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"${_scopeId}><div${_scopeId}><h2 class="font-semibold text-arbor-cream"${_scopeId}>xeno-canto</h2><p class="text-sm text-arbor-sage mt-1"${_scopeId}> Paquet de soumission avec fichier audio temporaire, espèce probable et coordonnées publiques. </p></div><button${ssrIncludeBooleanAttr(xenoCantoLoading.value) ? " disabled" : ""} class="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-arbor-emerald text-arbor-night text-sm font-semibold hover:bg-arbor-emerald-dark disabled:opacity-50 transition-colors"${_scopeId}><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 4v12m0-12l-4 4m4-4l4 4M4 20h16"${_scopeId}></path></svg> Préparer </button></div>`);
							if (xenoCantoError.value) _push(`<p class="mt-4 text-sm text-red-300"${_scopeId}>${ssrInterpolate(xenoCantoError.value)}</p>`);
							else _push(`<!---->`);
							if (xenoCanto.value) {
								_push(`<div class="mt-5 space-y-4"${_scopeId}><div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm"${_scopeId}><div class="rounded-lg bg-arbor-glass p-3"${_scopeId}><div class="text-xs text-arbor-sage mb-1"${_scopeId}>Espèce principale</div><div class="text-arbor-cream"${_scopeId}>${ssrInterpolate(xenoCanto.value.metadata.species.common_name || "À confirmer")}</div>`);
								if (xenoCanto.value.metadata.species.scientific_name) _push(`<div class="text-xs text-arbor-sage italic"${_scopeId}>${ssrInterpolate(xenoCanto.value.metadata.species.scientific_name)}</div>`);
								else _push(`<!---->`);
								_push(`</div><div class="rounded-lg bg-arbor-glass p-3"${_scopeId}><div class="text-xs text-arbor-sage mb-1"${_scopeId}>Lieu exporté</div><div class="text-arbor-cream"${_scopeId}>${ssrInterpolate(xenoCanto.value.metadata.location.name || "Lieu non renseigné")}</div>`);
								if (xenoCanto.value.metadata.location.latitude) _push(`<div class="text-xs text-arbor-sage"${_scopeId}>${ssrInterpolate(xenoCanto.value.metadata.location.latitude)}, ${ssrInterpolate(xenoCanto.value.metadata.location.longitude)}</div>`);
								else _push(`<!---->`);
								_push(`</div></div>`);
								if (xenoCanto.value.metadata.license_warning) _push(`<div class="rounded-lg border border-amber-400/30 bg-amber-400/10 p-3 text-sm text-amber-100"${_scopeId}>${ssrInterpolate(xenoCanto.value.metadata.license_warning)}</div>`);
								else _push(`<!---->`);
								_push(`<div class="flex flex-col sm:flex-row gap-3"${_scopeId}><button class="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-arbor-glass border border-arbor-glass-border text-arbor-sage text-sm hover:text-arbor-cream transition-colors"${_scopeId}> Copier l&#39;URL audio </button><button class="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-arbor-glass border border-arbor-glass-border text-arbor-sage text-sm hover:text-arbor-cream transition-colors"${_scopeId}> Ouvrir xeno-canto </button></div><div class="flex flex-col sm:flex-row gap-3"${_scopeId}><input${ssrRenderAttr("value", xenoCantoId.value)} type="text" inputmode="numeric" placeholder="ID xeno-canto" class="flex-1 rounded-lg border border-arbor-glass-border bg-arbor-night/60 px-3 py-2 text-sm text-arbor-cream placeholder:text-arbor-sage/60 focus:border-arbor-emerald focus:ring-arbor-emerald"${_scopeId}><button${ssrIncludeBooleanAttr(xenoCantoLoading.value) ? " disabled" : ""} class="inline-flex items-center justify-center px-4 py-2 rounded-lg bg-arbor-moss text-white text-sm font-semibold hover:bg-arbor-moss-light disabled:opacity-50 transition-colors"${_scopeId}> Marquer soumis </button></div>`);
								if (xenoCanto.value.xeno_canto_url) _push(`<a${ssrRenderAttr("href", xenoCanto.value.xeno_canto_url)} target="_blank" rel="noopener noreferrer" class="inline-flex text-sm text-arbor-emerald hover:text-arbor-cream transition-colors"${_scopeId}> Voir la fiche xeno-canto </a>`);
								else _push(`<!---->`);
								_push(`</div>`);
							} else _push(`<!---->`);
							_push(`</div>`);
						...
						_push(ssrRenderComponent(unref(Link), {
							...
							default: withCtx((_, _push, _parent, _scopeId) => {
								if (_push) {
									_push(`<div class="w-12 h-12 rounded-full bg-arbor-moss/30 flex items-center justify-center overflow-hidden ring-2 ring-arbor-glass-border/50 group-hover:ring-arbor-emerald/30 transition-colors"${_scopeId}>`);
									if (creatorAvatarUrl.value) _push(`<img${ssrRenderAttr("src", creatorAvatarUrl.value)}${ssrRenderAttr("alt", `Avatar de ${__props.sound.user?.name ?? "Anonyme"}`)} class="h-full w-full object-cover" loading="lazy"${_scopeId}>`);
									else _push(`<span class="text-lg font-display font-bold text-arbor-emerald"${_scopeId}>${ssrInterpolate(__props.sound.user?.name?.charAt(0)?.toUpperCase() ?? "?")}</span>`);
									_push(`</div><div${_scopeId}><div class="font-semibold text-arbor-cream group-hover:text-arbor-emerald transition-colors"${_scopeId}>${ssrInterpolate(__props.sound.user?.name ?? "Anonyme")}</div><div class="text-xs text-arbor-sage"${_scopeId}>Créateur</div></div>`);
								} else return [createVNode("div", { class: "w-12 h-12 rounded-full bg-arbor-moss/30 flex items-center justify-center overflow-hidden ring-2 ring-arbor-glass-border/50 group-hover:ring-arbor-emerald/30 transition-colors" }, [creatorAvatarUrl.value ? (openBlock(), createBlock("img", {
									key: 0,
									src: creatorAvatarUrl.value,
									alt: `Avatar de ${__props.sound.user?.name ?? "Anonyme"}`,
									class: "h-full w-full object-cover",
									loading: "lazy"
								}, null, 8, ["src", "alt"])) : (openBlock(), createBlock("span", {
									key: 1,
									class: "text-lg font-display font-bold text-arbor-emerald"
								}, toDisplayString(__props.sound.user?.name?.charAt(0)?.toUpperCase() ?? "?"), 1))]), createVNode("div", null, [createVNode("div", { class: "font-semibold text-arbor-cream group-hover:text-arbor-emerald transition-colors" }, toDisplayString(__props.sound.user?.name ?? "Anonyme"), 1), createVNode("div", { class: "text-xs text-arbor-sage" }, "Créateur")])];
							}),
							_: 1
						}, _parent, _scopeId));
					...
					} else return [createVNode("div", { class: "pt-24 pb-16" }, [createVNode("div", { class: "max-w-5xl mx-auto px-4 sm:px-6 lg:px-8" }, [
						createVNode(_sfc_main$8, { items: [
							{
								label: "Accueil",
								href: "/"
							},
						...
						createVNode("div", { class: "mb-8 trace-frame p-5 sm:p-8" }, [createVNode("div", { class: "relative z-10 grid gap-8 lg:grid-cols-[1fr_0.78fr] lg:items-end" }, [createVNode("div", null, [
							createVNode("p", { class: "atlas-kicker mb-4" }, "Fiche d'archive vivante · " + toDisplayString(archiveCode.value), 1),
							createVNode("h1", { class: "atlas-heading text-5xl sm:text-6xl" }, toDisplayString(__props.sound.title), 1),
							createVNode("p", { class: "mt-5 max-w-2xl text-arbor-sage leading-7" }, toDisplayString(__props.sound.description || "Une trace sonore publiée dans l’atlas Arborisis, à écouter comme un fragment de territoire."), 1)
						]), createVNode("div", { class: "grid gap-3 sm:grid-cols-2 lg:grid-cols-1" }, [createVNode("div", { class: "rounded-lg border border-arbor-mineral/10 bg-arbor-ink/40 p-4" }, [createVNode("div", { class: "text-xs text-arbor-sage" }, "Créateur"), createVNode(unref(Link), {
							href: _ctx.route("creators.show", __props.sound.user?.slug),
							class: "mt-1 block font-display text-2xl text-arbor-cream hover:text-arbor-lichen transition-colors"
						}, {
							default: withCtx(() => [createTextVNode(toDisplayString(__props.sound.user?.name ?? "Anonyme"), 1)]),
							_: 1
						}, 8, ["href"])]), createVNode("div", { class: "rounded-lg border border-arbor-mineral/10 bg-arbor-ink/40 p-4" }, [createVNode("div", { class: "text-xs text-arbor-sage" }, "Lieu public"), createVNode("div", { class: "mt-1 font-display text-2xl text-arbor-cream" }, toDisplayString(__props.sound.sound_location?.location_name || "Lieu approximé"), 1)])])])]),
						createVNode("div", { class: "grid grid-cols-1 lg:grid-cols-3 gap-8" }, [createVNode("div", { class: "lg:col-span-2 space-y-8" }, [
							createVNode("div", { class: "aspect-[16/9] rounded-xl overflow-hidden bg-arbor-deep relative group border border-arbor-mineral/10" }, [__props.coverUrl ? (openBlock(), createBlock("div", {
								key: 0,
							...
							createVNode("div", { class: "trace-frame p-6 relative" }, [createVNode("div", { class: "relative z-10" }, [
								createVNode("div", { class: "flex items-center gap-4 mb-4" }, [createVNode("button", {
									onClick: togglePlay,
									"aria-label": effectiveIsPlaying.value ? "Pause" : "Lire",
									class: ["audio-play-button w-14 h-14 shrink-0 group", { "is-playing": effectiveIsPlaying.value }]
									...
									fill: "currentColor",
									viewBox: "0 0 24 24"
								}, [createVNode("path", { d: "M6 4h4v16H6V4zm8 0h4v16h-4V4z" })]))], 10, ["aria-label"]), createVNode("div", { class: "flex-1 min-w-0" }, [createVNode("h2", { class: "font-display text-xl font-bold text-arbor-cream truncate" }, toDisplayString(__props.sound.title), 1), createVNode(unref(Link), {
									href: _ctx.route("creators.show", __props.sound.user?.slug),
									class: "text-sm text-arbor-sage hover:text-arbor-lichen transition-colors"
								}, {
									default: withCtx(() => [createTextVNode(toDisplayString(__props.sound.user?.name ?? "Anonyme"), 1)]),
							...
							createVNode("div", null, [createVNode("p", { class: "atlas-kicker mb-3" }, "Contexte de terrain"), createVNode("p", { class: "text-arbor-sage leading-relaxed whitespace-pre-line" }, toDisplayString(__props.sound.description || "Aucune description."), 1)]),
							createVNode("div", { class: "grid grid-cols-2 sm:grid-cols-3 gap-4" }, [
								__props.sound.category ? (openBlock(), createBlock("div", {
									key: 0,
									class: "field-stat hover-lift"
								...
								createVNode("div", { class: "field-stat hover-lift" }, [createVNode("div", { class: "flex items-center gap-2 mb-2" }, [(openBlock(), createBlock("svg", {
									class: "w-4 h-4 text-arbor-sage",
									fill: "none",
									stroke: "currentColor",
									viewBox: "0 0 24 24"
							...
							isOwner.value ? (openBlock(), createBlock("div", {
								...
								createVNode("div", { class: "flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4" }, [createVNode("div", null, [createVNode("h2", { class: "font-semibold text-arbor-cream" }, "xeno-canto"), createVNode("p", { class: "text-sm text-arbor-sage mt-1" }, " Paquet de soumission avec fichier audio temporaire, espèce probable et coordonnées publiques. ")]), createVNode("button", {
									onClick: prepareXenoCanto,
									disabled: xenoCantoLoading.value,
								...
								})])), createTextVNode(" Préparer ")], 8, ["disabled"])]),
								xenoCantoError.value ? (openBlock(), createBlock("p", {
									key: 0,
									class: "mt-4 text-sm text-red-300"
								}, toDisplayString(xenoCantoError.value), 1)) : createCommentVNode("", true),
								xenoCanto.value ? (openBlock(), createBlock("div", {
									key: 1,
									class: "mt-5 space-y-4"
								}, [
									createVNode("div", { class: "grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm" }, [createVNode("div", { class: "rounded-lg bg-arbor-glass p-3" }, [
										createVNode("div", { class: "text-xs text-arbor-sage mb-1" }, "Espèce principale"),
										createVNode("div", { class: "text-arbor-cream" }, toDisplayString(xenoCanto.value.metadata.species.common_name || "À confirmer"), 1),
										xenoCanto.value.metadata.species.scientific_name ? (openBlock(), createBlock("div", {
											key: 0,
											class: "text-xs text-arbor-sage italic"
										}, toDisplayString(xenoCanto.value.metadata.species.scientific_name), 1)) : createCommentVNode("", true)
									]), createVNode("div", { class: "rounded-lg bg-arbor-glass p-3" }, [
										createVNode("div", { class: "text-xs text-arbor-sage mb-1" }, "Lieu exporté"),
										createVNode("div", { class: "text-arbor-cream" }, toDisplayString(xenoCanto.value.metadata.location.name || "Lieu non renseigné"), 1),
										xenoCanto.value.metadata.location.latitude ? (openBlock(), createBlock("div", {
											key: 0,
											class: "text-xs text-arbor-sage"
										}, toDisplayString(xenoCanto.value.metadata.location.latitude) + ", " + toDisplayString(xenoCanto.value.metadata.location.longitude), 1)) : createCommentVNode("", true)
									])]),
									xenoCanto.value.metadata.license_warning ? (openBlock(), createBlock("div", {
										key: 0,
										class: "rounded-lg border border-amber-400/30 bg-amber-400/10 p-3 text-sm text-amber-100"
									}, toDisplayString(xenoCanto.value.metadata.license_warning), 1)) : createCommentVNode("", true),
									createVNode("div", { class: "flex flex-col sm:flex-row gap-3" }, [createVNode("button", {
										onClick: copyXenoCantoAudioUrl,
									...
									}, " Copier l'URL audio "), createVNode("button", {
										onClick: openXenoCantoUpload,
										class: "inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-arbor-glass border border-arbor-glass-border text-arbor-sage text-sm hover:text-arbor-cream transition-colors"
									}, " Ouvrir xeno-canto ")]),
									createVNode("div", { class: "flex flex-col sm:flex-row gap-3" }, [withDirectives(createVNode("input", {
										"onUpdate:modelValue": ($event) => xenoCantoId.value = $event,
										type: "text",
										inputmode: "numeric",
									...
									}, null, 8, ["onUpdate:modelValue"]), [[vModelText, xenoCantoId.value]]), createVNode("button", {
										onClick: markXenoCantoSubmitted,
										disabled: xenoCantoLoading.value,
									...
									}, " Marquer soumis ", 8, ["disabled"])]),
									xenoCanto.value.xeno_canto_url ? (openBlock(), createBlock("a", {
										key: 1,
										href: xenoCanto.value.xeno_canto_url,
										target: "_blank",
										rel: "noopener noreferrer",
										class: "inline-flex text-sm text-arbor-emerald hover:text-arbor-cream transition-colors"
									}, " Voir la fiche xeno-canto ", 8, ["href"])) : createCommentVNode("", true)
								])) : createCommentVNode("", true)
							])) : createCommentVNode("", true),
							...
							createVNode("div", { class: "glass-card p-6 hover-lift" }, [createVNode(unref(Link), {
								...
								default: withCtx(() => [createVNode("div", { class: "w-12 h-12 rounded-full bg-arbor-moss/30 flex items-center justify-center overflow-hidden ring-2 ring-arbor-glass-border/50 group-hover:ring-arbor-emerald/30 transition-colors" }, [creatorAvatarUrl.value ? (openBlock(), createBlock("img", {
									key: 0,
									src: creatorAvatarUrl.value,
									alt: `Avatar de ${__props.sound.user?.name ?? "Anonyme"}`,
									class: "h-full w-full object-cover",
