import { t as _sfc_main$2 } from "./AuthenticatedLayout-BFjA2KJR.js";
import { n as _sfc_main$3, r as _sfc_main$5, t as _sfc_main$4 } from "./TextInput-BIuc15x0.js";
...
var _sfc_main$1 = {
	...
	setup(__props, { emit: __emit }) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<div${ssrRenderAttrs(mergeProps({ class: "relative w-full min-h-[70vh] flex flex-col items-center justify-center overflow-hidden" }, _attrs))}><div class="pointer-events-none absolute inset-0 opacity-40" style="${ssrRenderStyle({ "background": "radial-gradient(ellipse 60% 40% at 50% 55%, rgba(143,230,193,0.08), transparent 70%)" })}"></div>`);
			...
			_push(`<div class="z-10 mb-10 flex w-full max-w-md flex-col items-center gap-4 px-6"><div class="relative w-full"><label class="mb-1.5 block text-center text-xs font-medium tracking-widest text-arbor-sage/70 uppercase"> Source audio </label><select${ssrIncludeBooleanAttr(unref(isRecording) || unref(isLoadingDevices)) ? " disabled" : ""} class="w-full appearance-none rounded-lg border border-arbor-mineral/10 bg-arbor-deep/60 px-4 py-2.5 text-sm text-arbor-cream shadow-sm backdrop-blur-sm transition-colors focus:border-arbor-lichen focus:outline-none focus:ring-1 focus:ring-arbor-lichen disabled:opacity-50"><!--[-->`);
			...
			_push(`<!--]--></select><div class="pointer-events-none absolute right-3 top-[2.1rem] text-arbor-sage/70"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></div></div><button class="text-xs text-arbor-sage/70 underline-offset-2 transition-colors hover:text-arbor-sage">${ssrInterpolate(showAdvanced.value ? "Masquer les options" : "Options avancées")}</button><div class="w-full overflow-hidden" style="${ssrRenderStyle(showAdvanced.value ? null : { display: "none" })}"><div class="flex flex-wrap justify-center gap-4 rounded-lg border border-arbor-mineral/10 bg-arbor-deep/40 px-4 py-3"><label class="flex cursor-pointer items-center gap-2 text-xs text-arbor-sage"><input${ssrIncludeBooleanAttr(Array.isArray(echoCancellation.value) ? ssrLooseContain(echoCancellation.value, null) : echoCancellation.value) ? " checked" : ""} type="checkbox" class="rounded border-arbor-glass-border bg-arbor-deep text-arbor-lichen focus:ring-arbor-lichen"> Réduction d&#39;écho </label><label class="flex cursor-pointer items-center gap-2 text-xs text-arbor-sage"><input${ssrIncludeBooleanAttr(Array.isArray(noiseSuppression.value) ? ssrLooseContain(noiseSuppression.value, null) : noiseSuppression.value) ? " checked" : ""} type="checkbox" class="rounded border-arbor-glass-border bg-arbor-deep text-arbor-lichen focus:ring-arbor-lichen"> Suppression de bruit </label><label class="flex cursor-pointer items-center gap-2 text-xs text-arbor-sage"><input${ssrIncludeBooleanAttr(Array.isArray(autoGainControl.value) ? ssrLooseContain(autoGainControl.value, null) : autoGainControl.value) ? " checked" : ""} type="checkbox" class="rounded border-arbor-glass-border bg-arbor-deep text-arbor-lichen focus:ring-arbor-lichen"> AGC </label></div></div></div><div class="relative z-10 mb-8 h-24 w-full max-w-2xl px-6"><canvas class="${ssrRenderClass([{ "opacity-100": state.value === "recording" || state.value === "paused" }, "h-full w-full rounded-xl opacity-0 transition-opacity duration-500"])}"></canvas></div><div class="relative z-10 flex flex-col items-center"><div class="${ssrRenderClass([{ "scale-110 opacity-0": state.value !== "idle" }, "absolute inset-0 -m-6 rounded-full border border-arbor-lichen/10 transition-all duration-700"])}"></div><div class="${ssrRenderClass([{ "scale-110 opacity-0": state.value !== "idle" }, "absolute inset-0 -m-12 rounded-full border border-arbor-firefly/10 transition-all duration-700 delay-100"])}"></div><div class="absolute inset-0 -m-4 rounded-full border border-arbor-firefly/35 transition-all duration-300" style="${ssrRenderStyle({
				transform: state.value === "recording" ? `scale(${1 + unref(peak) * .08})` : "scale(1)",
				opacity: state.value === "recording" ? .35 + unref(peak) * .35 : 0
			})}"></div><div class="absolute inset-0 -m-8 rounded-full border border-arbor-lichen/18 transition-all duration-500" style="${ssrRenderStyle({
				transform: state.value === "recording" ? `scale(${1 + unref(rms) * .12})` : "scale(1)",
				opacity: state.value === "recording" ? .24 + unref(rms) * .28 : 0
			})}"></div><button class="${ssrRenderClass([[state.value === "recording" ? "bg-arbor-firefly/10 shadow-[0_0_34px_rgba(143,230,193,0.18)]" : state.value === "paused" ? "bg-arbor-amber/10 shadow-[0_0_40px_rgba(212,165,116,0.15)]" : "bg-arbor-glass shadow-[0_0_28px_rgba(215,180,106,0.08)] hover:shadow-[0_0_42px_rgba(143,230,193,0.12)] hover:bg-arbor-lichen/5"], "relative flex h-28 w-28 items-center justify-center rounded-full transition-all duration-300 active:scale-[0.96]"])}">`);
			if (state.value === "idle") _push(`<svg class="h-10 w-10 text-arbor-lichen transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z"></path></svg>`);
			else if (state.value === "recording") _push(`<svg class="h-10 w-10 text-arbor-firefly" fill="currentColor" viewBox="0 0 24 24"><rect x="6" y="4" width="4" height="16" rx="1"></rect><rect x="14" y="4" width="4" height="16" rx="1"></rect></svg>`);
			...
			_push(`</button><div class="${ssrRenderClass([{
				"text-arbor-firefly": state.value === "recording",
				"text-arbor-amber": state.value === "paused",
				"text-arbor-sage/70": state.value === "idle"
			}, "mt-5 font-mono text-3xl tracking-widest transition-colors duration-300"])}">${ssrInterpolate(timerDisplay.value)}</div><p class="mt-2 text-sm font-display italic text-arbor-sage/70 transition-all duration-300">`);
...
var _sfc_main = {
	...
	setup(__props) {
		...
		const onConfirmRecording = (blob, url) => {
			...
			const filename = `arborisis_record_${y}${m}${d}_${h}${min}${String(now.getSeconds()).padStart(2, "0")}.${blob.type.includes("mp4") || blob.type.includes("aac") ? "m4a" : "webm"}`;
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(ssrRenderComponent(_sfc_main$2, null, {
				header: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(`<div${_scopeId}><p class="atlas-kicker"${_scopeId}>Capture directe</p><h2 class="font-display text-2xl font-semibold text-arbor-cream"${_scopeId}> Enregistrer une trace sonore </h2></div>`);
					else return [createVNode("div", null, [createVNode("p", { class: "atlas-kicker" }, "Capture directe"), createVNode("h2", { class: "font-display text-2xl font-semibold text-arbor-cream" }, " Enregistrer une trace sonore ")])];
				}),
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						_push(`<div class="relative min-h-[calc(100vh-8rem)]"${_scopeId}>`);
						if (step.value === "record") {
							_push(`<div class="py-6"${_scopeId}><div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8"${_scopeId}><div class="trace-frame mb-8 p-6 text-center sm:p-8"${_scopeId}><div class="relative z-10"${_scopeId}><p class="atlas-kicker mb-4"${_scopeId}>Le terrain commence ici</p><h1 class="atlas-heading text-5xl sm:text-6xl"${_scopeId}> Capturez l&#39;instant. </h1><p class="mx-auto mt-5 max-w-xl text-sm leading-6 text-arbor-sage/80"${_scopeId}> Choisissez votre source, écoutez le lieu, puis gardez uniquement les captures qui méritent d&#39;entrer dans l&#39;atlas. </p>`);
							if (__props.selectedListeningPoint) _push(`<p class="mx-auto mt-4 max-w-xl rounded-lg border border-arbor-lichen/20 bg-arbor-lichen/10 px-4 py-3 text-sm text-arbor-cream"${_scopeId}> Nouvelle prise pour <span class="font-semibold text-arbor-lichen"${_scopeId}>${ssrInterpolate(__props.selectedListeningPoint.title)}</span>. </p>`);
							else _push(`<!---->`);
							_push(`</div></div><div class="trace-frame overflow-hidden"${_scopeId}><div class="relative z-10"${_scopeId}>`);
							_push(ssrRenderComponent(_sfc_main$1, { onConfirm: onConfirmRecording }, null, _parent, _scopeId));
							_push(`</div></div></div></div>`);
						} else _push(`<!---->`);
						if (step.value === "meta") {
							_push(`<div class="py-8"${_scopeId}><div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8"${_scopeId}><div class="mb-6 flex items-center justify-between"${_scopeId}><div${_scopeId}><p class="atlas-kicker"${_scopeId}>Contexte de terrain</p><h2 class="font-display text-3xl font-semibold text-arbor-cream"${_scopeId}> Décrivez votre capture </h2><p class="mt-1 text-sm text-arbor-sage/70"${_scopeId}> Vous pouvez revenir en arrière pour réenregistrer. </p></div><button class="btn-secondary text-sm"${_scopeId}><svg class="mr-1.5 inline h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"${_scopeId}></path></svg> Retour </button></div><form class="space-y-8"${_scopeId}><input type="hidden"${ssrRenderAttr("value", unref(form).listening_point_id)}${_scopeId}><input type="hidden"${ssrRenderAttr("value", unref(form).create_new_listening_point)}${_scopeId}>`);
							if (__props.selectedListeningPoint) _push(`<div class="trace-frame border-arbor-lichen/25 bg-arbor-lichen/10 p-5"${_scopeId}><div class="relative z-10"${_scopeId}><p class="atlas-kicker mb-2"${_scopeId}>Point d&#39;écoute prérempli</p><h3 class="font-display text-2xl font-semibold text-arbor-cream"${_scopeId}>${ssrInterpolate(__props.selectedListeningPoint.title)}</h3><p class="mt-1 text-sm text-arbor-sage"${_scopeId}> La capture sera attachée à ce point après publication. </p></div></div>`);
							else _push(`<!---->`);
							_push(`<div class="trace-frame p-6"${_scopeId}><div class="relative z-10"${_scopeId}>`);
							_push(ssrRenderComponent(_sfc_main$3, { value: "Enregistrement" }, null, _parent, _scopeId));
							_push(`<div class="mt-3 flex items-center gap-4 rounded-lg border border-arbor-mineral/10 bg-arbor-deep/40 px-4 py-3"${_scopeId}><div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-arbor-lichen/10"${_scopeId}><svg class="h-5 w-5 text-arbor-lichen" fill="none" stroke="currentColor" viewBox="0 0 24 24"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z"${_scopeId}></path></svg></div><div class="min-w-0 flex-1"${_scopeId}><p class="truncate text-sm text-arbor-cream"${_scopeId}>${ssrInterpolate(unref(form).audio_file?.name || "Enregistrement")}</p><p class="text-xs text-arbor-sage/70"${_scopeId}>${ssrInterpolate(unref(form).audio_file?.type?.includes("mp4") || unref(form).audio_file?.type?.includes("aac") ? "M4A" : "WebM")} — ${ssrInterpolate((unref(form).audio_file?.size / 1024 / 1024).toFixed(2))} Mo </p></div><audio${ssrRenderAttr("src", recordedUrl.value)} controls class="h-8 w-32 sm:w-48"${_scopeId}></audio></div></div></div><div class="trace-frame p-5 sm:p-6"${_scopeId}><div class="relative z-10"${_scopeId}>`);
							...
							_push(`</div></div><div class="trace-frame p-6"${_scopeId}><div class="relative z-10"${_scopeId}><div class="mb-4 flex items-start justify-between gap-4"${_scopeId}><div${_scopeId}><p class="atlas-kicker mb-1"${_scopeId}>Lieu d&#39;écoute</p><h3 class="font-display text-2xl font-semibold text-arbor-cream"${_scopeId}>Localisation *</h3></div><span class="rounded-full border border-arbor-firefly/20 bg-arbor-firefly/10 px-3 py-1 text-[11px] text-arbor-firefly"${_scopeId}> Public approximé </span></div><div class="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2"${_scopeId}><div${_scopeId}>`);
							_push(ssrRenderComponent(_sfc_main$3, {
								for: "latitude",
								value: "Latitude"
							}, null, _parent, _scopeId));
							...
							_push(`</div></div><button type="button"${ssrIncludeBooleanAttr(locating.value) ? " disabled" : ""} class="mb-2 flex items-center gap-1.5 text-sm text-arbor-lichen transition-colors hover:text-arbor-firefly disabled:cursor-not-allowed disabled:opacity-50"${_scopeId}>`);
							...
							_push(ssrRenderComponent(_sfc_main$5, { message: unref(form).errors.category_id || unref(form).errors.new_category_name }, null, _parent, _scopeId));
							_push(`</div><div${_scopeId}>`);
							_push(ssrRenderComponent(_sfc_main$3, {
								for: "environment_id",
								value: "Environnement"
							...
							_push(ssrRenderComponent(_sfc_main$5, { message: unref(form).errors.environment_id || unref(form).errors.new_environment_name }, null, _parent, _scopeId));
							_push(`</div></div><div class="grid grid-cols-1 gap-6 sm:grid-cols-2"${_scopeId}><div${_scopeId}>`);
							_push(ssrRenderComponent(_sfc_main$3, {
								for: "equipment",
						...
						default: withCtx(() => [step.value === "record" ? (openBlock(), createBlock("div", {
							key: 0,
							class: "py-6"
						}, [createVNode("div", { class: "mx-auto max-w-4xl px-4 sm:px-6 lg:px-8" }, [createVNode("div", { class: "trace-frame mb-8 p-6 text-center sm:p-8" }, [createVNode("div", { class: "relative z-10" }, [
							createVNode("p", { class: "atlas-kicker mb-4" }, "Le terrain commence ici"),
							createVNode("h1", { class: "atlas-heading text-5xl sm:text-6xl" }, " Capturez l'instant. "),
							createVNode("p", { class: "mx-auto mt-5 max-w-xl text-sm leading-6 text-arbor-sage/80" }, " Choisissez votre source, écoutez le lieu, puis gardez uniquement les captures qui méritent d'entrer dans l'atlas. "),
							__props.selectedListeningPoint ? (openBlock(), createBlock("p", {
								key: 0,
								class: "mx-auto mt-4 max-w-xl rounded-lg border border-arbor-lichen/20 bg-arbor-lichen/10 px-4 py-3 text-sm text-arbor-cream"
							}, [
								createTextVNode(" Nouvelle prise pour "),
								createVNode("span", { class: "font-semibold text-arbor-lichen" }, toDisplayString(__props.selectedListeningPoint.title), 1),
								createTextVNode(". ")
							])) : createCommentVNode("", true)
						])]), createVNode("div", { class: "trace-frame overflow-hidden" }, [createVNode("div", { class: "relative z-10" }, [createVNode(_sfc_main$1, { onConfirm: onConfirmRecording })])])])])) : createCommentVNode("", true)]),
						_: 1
						...
						default: withCtx(() => [step.value === "meta" ? (openBlock(), createBlock("div", {
							key: 0,
							class: "py-8"
						}, [createVNode("div", { class: "mx-auto max-w-3xl px-4 sm:px-6 lg:px-8" }, [createVNode("div", { class: "mb-6 flex items-center justify-between" }, [createVNode("div", null, [
							createVNode("p", { class: "atlas-kicker" }, "Contexte de terrain"),
							createVNode("h2", { class: "font-display text-3xl font-semibold text-arbor-cream" }, " Décrivez votre capture "),
							createVNode("p", { class: "mt-1 text-sm text-arbor-sage/70" }, " Vous pouvez revenir en arrière pour réenregistrer. ")
						]), createVNode("button", {
							...
							__props.selectedListeningPoint ? (openBlock(), createBlock("div", {
								...
								createVNode("p", { class: "atlas-kicker mb-2" }, "Point d'écoute prérempli"),
								createVNode("h3", { class: "font-display text-2xl font-semibold text-arbor-cream" }, toDisplayString(__props.selectedListeningPoint.title), 1),
								createVNode("p", { class: "mt-1 text-sm text-arbor-sage" }, " La capture sera attachée à ce point après publication. ")
							])])) : createCommentVNode("", true),
							createVNode("div", { class: "trace-frame p-6" }, [createVNode("div", { class: "relative z-10" }, [createVNode(_sfc_main$3, { value: "Enregistrement" }), createVNode("div", { class: "mt-3 flex items-center gap-4 rounded-lg border border-arbor-mineral/10 bg-arbor-deep/40 px-4 py-3" }, [
								createVNode("div", { class: "flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-arbor-lichen/10" }, [(openBlock(), createBlock("svg", {
									class: "h-5 w-5 text-arbor-lichen",
									fill: "none",
									stroke: "currentColor",
									viewBox: "0 0 24 24"
								}, [createVNode("path", {
								...
								createVNode("div", { class: "min-w-0 flex-1" }, [createVNode("p", { class: "truncate text-sm text-arbor-cream" }, toDisplayString(unref(form).audio_file?.name || "Enregistrement"), 1), createVNode("p", { class: "text-xs text-arbor-sage/70" }, toDisplayString(unref(form).audio_file?.type?.includes("mp4") || unref(form).audio_file?.type?.includes("aac") ? "M4A" : "WebM") + " — " + toDisplayString((unref(form).audio_file?.size / 1024 / 1024).toFixed(2)) + " Mo ", 1)]),
								createVNode("audio", {
							...
							createVNode("div", { class: "trace-frame p-6" }, [createVNode("div", { class: "relative z-10" }, [
								createVNode("div", { class: "mb-4 flex items-start justify-between gap-4" }, [createVNode("div", null, [createVNode("p", { class: "atlas-kicker mb-1" }, "Lieu d'écoute"), createVNode("h3", { class: "font-display text-2xl font-semibold text-arbor-cream" }, "Localisation *")]), createVNode("span", { class: "rounded-full border border-arbor-firefly/20 bg-arbor-firefly/10 px-3 py-1 text-[11px] text-arbor-firefly" }, " Public approximé ")]),
							...
							createVNode("div", { class: "grid grid-cols-1 gap-6 sm:grid-cols-2" }, [createVNode("div", null, [
								...
								createVNode(_sfc_main$5, { message: unref(form).errors.category_id || unref(form).errors.new_category_name }, null, 8, ["message"])
							]), createVNode("div", null, [
								createVNode(_sfc_main$3, {
									for: "environment_id",
									value: "Environnement"
								...
								createVNode(_sfc_main$5, { message: unref(form).errors.environment_id || unref(form).errors.new_environment_name }, null, 8, ["message"])
							])]),
							createVNode("div", { class: "grid grid-cols-1 gap-6 sm:grid-cols-2" }, [createVNode("div", null, [createVNode(_sfc_main$3, {
								for: "equipment",
								value: "Matériel utilisé"
