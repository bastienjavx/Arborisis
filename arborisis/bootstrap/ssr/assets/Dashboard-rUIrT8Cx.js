		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (__props.quests.length > 0) {
				...
				ssrRenderList(__props.quests, (quest, index) => {
					_push(`<div class="group relative bg-arbor-charcoal/50 border border-arbor-fog/50 rounded-xl p-4 hover:border-arbor-moss/40 hover:bg-arbor-charcoal/70 transition-colors duration-300" style="${ssrRenderStyle(`animation: slideInRight 0.4s ease-out forwards; animation-delay: ${index * .1}s; opacity: 0;`)}"><div class="flex items-start justify-between gap-3 mb-3"><div class="flex-1 min-w-0"><div class="flex items-center gap-2 mb-1"><h3 class="text-arbor-cream text-sm font-medium truncate group-hover:text-arbor-emerald transition-colors">${ssrInterpolate(quest.title)}</h3>`);
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (__props.medals.length > 0) {
				...
				ssrRenderList(__props.medals, (medal, index) => {
					_push(`<div class="${ssrRenderClass([getRarityClass(medal.rarity), "group relative flex flex-col items-center text-center p-2 rounded-xl bg-arbor-charcoal/40 border hover:border-arbor-amber/30 hover:bg-arbor-charcoal/60 transition-transform duration-300 cursor-default"])}" style="${ssrRenderStyle(`animation: scaleIn 0.4s ease-out forwards; animation-delay: ${index * .08}s; opacity: 0;`)}"><div class="w-10 h-10 rounded-full bg-arbor-night/60 border border-current/20 flex items-center justify-center mb-1.5 group-hover:scale-110 transition-transform"><span class="text-lg">${ssrInterpolate(medal.icon || "🎖️")}</span></div><span class="text-[10px] font-medium text-arbor-cream leading-tight line-clamp-2">${ssrInterpolate(medal.name)}</span><div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2.5 py-1.5 bg-arbor-night border border-arbor-fog rounded-lg shadow-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20 w-32"><p class="text-arbor-cream text-xs font-medium">${ssrInterpolate(medal.name)}</p><p class="text-arbor-sage text-[10px] mt-0.5">${ssrInterpolate(medal.description)}</p><p class="text-arbor-sage/60 text-[10px] mt-1">${ssrInterpolate(formatDate(medal.unlocked_at))}</p></div></div>`);
				});
				_push(`<!--]--></div>`);
