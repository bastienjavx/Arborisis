...
var _sfc_main = {
	...
	setup(__props) {
		const SoundMap = defineAsyncComponent(() => import("./SoundMap-Cm5wH1NE.js"));
		const props = __props;
		const sounds = ref([]);
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(ssrRenderComponent(_sfc_main$1, null, {
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						_push(`<div class="relative isolate h-[calc(100vh-4rem)] min-h-[600px] overflow-hidden"${_scopeId}><div class="absolute inset-0 z-[5] pointer-events-none bg-gradient-radial from-arbor-moss/10 via-transparent to-transparent"${_scopeId}></div><div class="absolute inset-0 z-[6] pointer-events-none map-grain"${_scopeId}></div><div class="${ssrRenderClass([{ "map-title-hidden": titleHidden.value }, "absolute top-6 left-1/2 -translate-x-1/2 z-[40] text-center map-title-fade pointer-events-none"])}"${_scopeId}><h1 class="font-display text-3xl md:text-4xl font-semibold text-arbor-cream tracking-tight"${_scopeId}> Carte sonore </h1><p class="text-arbor-sage/70 text-sm mt-1 font-light"${_scopeId}> Explorez les enregistrements à travers le monde </p></div><button class="${ssrRenderClass([{ "ring-2 ring-arbor-emerald/40": sidebarOpen.value }, "md:hidden fixed bottom-6 left-4 z-[40] w-12 h-12 rounded-xl glass-card flex items-center justify-center shadow-lg shadow-black/30"])}" aria-label="Ouvrir le panneau de l&#39;explorateur"${_scopeId}>`);
						...
						_push(`</button><div class="${ssrRenderClass([sidebarOpen.value ? "mobile-sidebar-open" : "mobile-sidebar-closed md:mobile-sidebar-open", "absolute top-4 left-4 z-[40] w-[340px] lg:w-[380px] max-w-[calc(100vw-2rem)] mobile-sidebar"])}"${_scopeId}><div class="glass-card shadow-2xl shadow-black/25 overflow-hidden flex flex-col max-h-[calc(100vh-7rem)]"${_scopeId}><div class="p-4 border-b border-arbor-glass-border animate-fade-in-up"${_scopeId}><div class="relative"${_scopeId}><svg class="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-arbor-sage/60" fill="none" stroke="currentColor" viewBox="0 0 24 24"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"${_scopeId}></path></svg><input${ssrRenderAttr("value", searchQuery.value)} type="text" placeholder="Rechercher un son, un lieu..." class="w-full pl-10 pr-9 py-2.5 bg-arbor-charcoal/60 border border-arbor-fog/40 rounded-xl text-sm text-arbor-cream placeholder:text-arbor-sage/40 focus:outline-none focus:border-arbor-emerald/50 focus:ring-2 focus:ring-arbor-emerald/10 transition-all search-pulse" aria-label="Rechercher un son ou un lieu"${_scopeId}>`);
						if (searchQuery.value) _push(`<button class="absolute right-3 top-1/2 -translate-y-1/2 text-arbor-sage/50 hover:text-arbor-cream transition-colors" aria-label="Effacer la recherche"${_scopeId}><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"${_scopeId}></path></svg></button>`);
						else _push(`<!---->`);
						_push(`</div></div><div class="px-4 py-3 border-b border-arbor-glass-border animate-fade-in-up stagger-1"${_scopeId}><div class="flex flex-wrap gap-1.5"${_scopeId}><button class="${ssrRenderClass(["px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-200", selectedCategory.value === "" ? "bg-arbor-emerald/15 text-arbor-emerald border border-arbor-emerald/30 shadow-sm shadow-arbor-emerald/5" : "bg-arbor-charcoal/60 text-arbor-sage border border-arbor-fog/40 hover:border-arbor-sage/50 hover:bg-arbor-charcoal"])}"${_scopeId}> Tous </button><!--[-->`);
						ssrRenderList(__props.categories, (category) => {
							_push(`<button class="${ssrRenderClass(["px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-200 border", selectedCategory.value == category.id ? `${getCategoryStyle(category.name).bg} ${getCategoryStyle(category.name).text} ${getCategoryStyle(category.name).border} shadow-sm ${getCategoryStyle(category.name).glow}` : "bg-arbor-charcoal/60 text-arbor-sage border-arbor-fog/40 hover:border-arbor-sage/50 hover:bg-arbor-charcoal"])}"${_scopeId}>${ssrInterpolate(category.name)}</button>`);
						});
						...
						if (initialLoad.value) _push(`<span class="text-arbor-sage/70"${_scopeId}>Chargement...</span>`);
							...
							_push(`<span class="text-arbor-sage"${_scopeId}><span class="text-arbor-emerald font-semibold tabular-nums"${_scopeId}>${ssrInterpolate(sounds.value.length)}</span> son${ssrInterpolate(sounds.value.length > 1 ? "s" : "")} trouvé${ssrInterpolate(sounds.value.length > 1 ? "s" : "")} `);
						...
						if (hasSounds.value && !initialLoad.value) {
						...
						createVNode("div", { class: ["absolute top-6 left-1/2 -translate-x-1/2 z-[40] text-center map-title-fade pointer-events-none", { "map-title-hidden": titleHidden.value }] }, [createVNode("h1", { class: "font-display text-3xl md:text-4xl font-semibold text-arbor-cream tracking-tight" }, " Carte sonore "), createVNode("p", { class: "text-arbor-sage/70 text-sm mt-1 font-light" }, " Explorez les enregistrements à travers le monde ")], 2),
						createVNode("button", {
							class: ["md:hidden fixed bottom-6 left-4 z-[40] w-12 h-12 rounded-xl glass-card flex items-center justify-center shadow-lg shadow-black/30", { "ring-2 ring-arbor-emerald/40": sidebarOpen.value }],
						...
						createVNode("div", {
							class: ["absolute top-4 left-4 z-[40] w-[340px] lg:w-[380px] max-w-[calc(100vw-2rem)] mobile-sidebar", sidebarOpen.value ? "mobile-sidebar-open" : "mobile-sidebar-closed md:mobile-sidebar-open"],
							onMouseenter: hideTitle,
							onClick: hideTitle
						}, [createVNode("div", { class: "glass-card shadow-2xl shadow-black/25 overflow-hidden flex flex-col max-h-[calc(100vh-7rem)]" }, [
							createVNode("div", { class: "p-4 border-b border-arbor-glass-border animate-fade-in-up" }, [createVNode("div", { class: "relative" }, [
								(openBlock(), createBlock("svg", {
							...
							createVNode("div", { class: "px-4 py-3 border-b border-arbor-glass-border animate-fade-in-up stagger-1" }, [createVNode("div", { class: "flex flex-wrap gap-1.5" }, [createVNode("button", {
								class: ["px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-200", selectedCategory.value === "" ? "bg-arbor-emerald/15 text-arbor-emerald border border-arbor-emerald/30 shadow-sm shadow-arbor-emerald/5" : "bg-arbor-charcoal/60 text-arbor-sage border border-arbor-fog/40 hover:border-arbor-sage/50 hover:bg-arbor-charcoal"],
								...
								return openBlock(), createBlock("button", {
									key: category.id,
									class: ["px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-200 border", selectedCategory.value == category.id ? `${getCategoryStyle(category.name).bg} ${getCategoryStyle(category.name).text} ${getCategoryStyle(category.name).border} shadow-sm ${getCategoryStyle(category.name).glow}` : "bg-arbor-charcoal/60 text-arbor-sage border-arbor-fog/40 hover:border-arbor-sage/50 hover:bg-arbor-charcoal"],
							...
							createVNode("div", { class: "px-4 py-2.5 border-b border-arbor-glass-border flex items-center justify-between animate-fade-in-up stagger-2" }, [createVNode("span", { class: "text-xs" }, [initialLoad.value ? (openBlock(), createBlock("span", {
								...
								createVNode("span", { class: "text-arbor-emerald font-semibold tabular-nums" }, toDisplayString(sounds.value.length), 1),
								createTextVNode(" son" + toDisplayString(sounds.value.length > 1 ? "s" : "") + " trouvé" + toDisplayString(sounds.value.length > 1 ? "s" : "") + " ", 1),
							...
							createVNode("div", { class: "flex-1 overflow-y-auto min-h-0 custom-scrollbar" }, [hasSounds.value && !initialLoad.value ? (openBlock(), createBlock("div", {
								key: 0,
								...
								createVNode("p", { class: "text-xs text-arbor-sage/50 mb-4" }, "Essayez une autre recherche ou catégorie"),
								createVNode("button", {
