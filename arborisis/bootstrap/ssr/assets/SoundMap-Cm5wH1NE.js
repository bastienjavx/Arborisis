/* empty css                 */
import { mergeProps, onMounted, onUnmounted, ref, useSSRContext, watch } from "vue";
...
var _sfc_main = {
    ...
    `;
		...
		const updateMarkers = () => {
			...
			map.value.fitBounds(group.getBounds().pad(.15), {
				animate: true,
				duration: 1
			});
		};
		...
		const highlightMarker = (soundId) => {
			if (!markersMap.value.get(String(soundId))) return;
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (__props.sounds.length === 0) _push(`<div class="absolute inset-0 z-[30] flex items-center justify-center pointer-events-none"><div class="text-center animate-scale-in"><div class="inline-flex items-center justify-center w-24 h-24 rounded-3xl bg-arbor-moss/10 mb-6 ring-1 ring-arbor-emerald/10"><div class="relative"><div class="absolute inset-0 rounded-full bg-arbor-emerald/20 animate-ping" style="${ssrRenderStyle({ "animation-duration": "3s" })}"></div><svg class="w-12 h-12 text-arbor-moss/50 relative" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 7m0 13V7m0 0L9 7"></path></svg></div></div><p class="text-arbor-sage text-base font-medium">Aucun son sur cette zone</p><p class="text-arbor-sage/50 text-sm mt-2 max-w-xs mx-auto">Explorez d&#39;autres catégories ou élargissez votre recherche pour découvrir des enregistrements.</p></div></div>`);
			else _push(`<!---->`);
			_push(`</div>`);
		};
	}
