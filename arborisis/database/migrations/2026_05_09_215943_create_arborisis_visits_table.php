...
return new class extends Migration
{
    ...
    {
        Schema::create('<redacted>_visits', function (Blueprint $table) {
            $table->id();
            ...
            $table->index(['user_id', '<redacted>_point_id']);
            $table->index(['user_id', 'visited_at']);
            $table->index('<redacted>_point_id');
            $table->index('status');
            $table->unique(['user_id', '<redacted>_point_id', 'visited_at'], 'unique_visit_per_day');
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('<redacted>_visits');
    }
};
